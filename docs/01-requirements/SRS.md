...
UniDipVeri is a new, self-contained system. It integrates with walt.id for VC issuance and verification but owns its own domain model (students, programs, credentials, shares, audit log) independently of walt.id's internal representations. UniDipVeri receives academic records from an authoritative academic source system. Source academic records are assumed to be correct and authentic at the point of import. UniDipVeri does not independently authenticate individual grades or transcript entries. Instead, it evaluates imported academic data against configured graduation eligibility rules. Registrar users review and initiate credential issuance requests, while authorized Approvers authorize issuance before the credential is cryptographically issued.
...
- Maintain an audit trail of issuance, approval, revocation, sharing, and verification events.
...
| Student / Graduate     | Credential holder                                                                                                               | Low; general web user           |
| Verifier               | External employer or organization checking a credential                                                                         | None; anonymous, no account     |
| Platform Administrator | Configures university-level settings, schema, and approval policy                                                               | Moderate                        |

A single staff account may hold both the Registrar and Approver roles in the MVP, but the system shall treat them as distinct permissions so that a stricter policy (e.g., requiring a different person to approve) can be enabled without a redesign.
...
- **AS-03** — walt.id's issuer and verifier services are available and correctly configured with MIU's issuer identity/keys before any issuance or verification can occur.
...
- The system shall not implement a custom wallet; credential holders view credentials through the UniDipVeri student portal only.
...
1. Academic record import/ingestion from the designated academic source.
2. Graduation eligibility rule evaluation.
3. Eligibility result and failed-requirement reporting.
4. University staff authentication and role-based access (Registrar, Approver, Platform Administrator).
5. Student record management.
6. Academic program management.
7. Credential issuance requests with a configurable approval policy (MVP policy: 1 required approver).
8. VC generation via walt.id upon approval.
9. Student credential viewing.
10. Creation of short-lived, revocable verification share links.
11. Public verification without verifier registration.
12. Cryptographic credential verification (issuer + integrity + status), delegated to walt.id.
13. Credential revocation.
14. Credential reissuance with lineage to the superseded credential.
15. Verification, issuance, approval, and sharing event logging.
16. Basic audit history views for registrar and student.
...
- **FR-STU-02** Registrar users shall be able to view imported student records.
- **FR-STU-03** A student record shall contain at minimum: student ID, name, email, program, enrollment/graduation information.
- **FR-STU-04** Students shall only be able to view their own credentials.
- **FR-STU-05** The system shall treat all student data entered by the Registrar as authoritative input (see AS-01); it shall not perform independent eligibility verification.
...
This workflow is new relative to v1.0: issuance is split into a **request** step and a **signing** step, gated by an approval policy.
...
- **FR-CRED-03** The system shall use walt.id as the VC infrastructure for credential issuance (see Architecture_Design.md).
...
- **FR-CRED-06** The system shall prevent duplicate issuance of the same diploma unless explicitly performing a reissuance (see 4.9).
...
- **FR-CRED-12** The system shall support issuing a corrected credential after revocation, subject to the same approval workflow as a new issuance (4.5).
...
- The frontend shall never call walt.id directly; all VC operations shall be mediated by the application's own API (see Architecture_Design.md, NFR-05).
...
The entity model (University, Program, Student, CredentialSchema, CredentialIssuanceRequest, CredentialApproval, ApprovalPolicy, Credential, Share, VerificationEvent) and its ERD are specified in `Data_Model.md`. At the SRS level, the system shall persist the entities implied by Sections 4.2–4.12 with sufficient fidelity to satisfy the audit requirements in Section 4.12 and NFR-06.

---

## 8. Acceptance Criteria

- **AC-01 — Request & Approve.** Given an eligible student, when a Registrar creates an issuance request and the required number of Approvers (1, in the MVP) approve it, then a valid VC-backed credential is created.
...
- **AC-03 — Rejection.** Given a pending issuance request, when an Approver rejects it, then no credential is issued and the request is marked `REJECTED`.
- **AC-04 — Student access.** Given a student with an issued credential, when they log in, then they can view the credential.
- **AC-05 — Share.** Given an active credential, when the student creates a share, then the system generates an opaque public URL with an expiration time.
- **AC-06 — Verification.** Given an active share, when an unauthenticated verifier opens it, then the system verifies the credential and displays the relevant information.
- **AC-07 — Revocation.** Given a revoked credential, when a verifier attempts verification, then the system displays `REVOKED`.
- **AC-08 — Expired share.** Given an expired share, when a verifier opens it, then credential information is not displayed and the system reports `EXPIRED_SHARE`.
- **AC-09 — Reissue.** Given a revoked credential requiring correction, when the Registrar requests reissuance and it is approved, then a new valid credential is created and linked to the previous credential.
- **AC-10 — Single tenant.** The system does not expose any function for creating or switching between universities.
- **AC-11 — Trust boundary.** The verification result never implies that UniDipVeri independently re-checked the graduate's academic performance; it only attests to credential authenticity and status.
